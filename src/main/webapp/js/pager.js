function Page(opt){
		...
		var clickpages = {
			...
			init:function(){
				...
				this.elem.children('li').click(function () {
					var txt = $(this).children('a').text();//点击的page
					var page = '', ele = null;
					var page1 = parseInt(clickpages.elem.children('li.active').attr('page'));//目前选中的page
					if (isNaN(parseInt(txt))) {
						switch (txt) {
							case '下一页':
								if (page1 == clickpages.num) {//clickpages.num页码总数
									return;
								}
								if (page1 >= (clickpages.num - 2) || clickpages.num <= 6 || page1 < 3) {
									ele = clickpages.elem.children('li.active').next();
									var a=ele.find("a").html();
			...
			newPages:function (type, i) {
				...
				var HL = '<li><a>...</a></li>';
				html = '<li><a href="./1.html" aria-label="Previous">&laquo;</a></li>\
					<li><a href="./'+i+'.html">上一页</a></li>'
				for (var n = 0;n<3;n++){
					htmlC += '<li '+((n-1)==0?'class="active"':'')+' page="'+(i+n-1)+'"><a href="./'+(i+n-1)+'.html">'+(i+n-1)+'</a></li>';
					htmlLeft += '<li '+((n+2)==i?'class="active"':'')+' page="'+(n+2)+'"><a href="./'+(n+2)+'.html">'+(n+2)+'</a></li>';
					htmlRight += '<li '+((set.num+n-3)==i?'class="active"':'')+' page="'+(set.num+n-3)+'"><a href="./'+(set.num+n-3)+'.html">'+(set.num+n-3)+'</a></li>';
				}
				
				switch (type) {
					case "next":
						if(i<=4){
							html+='<li page="1"><a href="./1.html">1</a></li>'+htmlLeft+HL+'<li page="'+set.num+'"><a href="./'+set.num+'.html">'+set.num+'</a></li>';
						}else if(i>=(set.num-3)){
							html+='<li page="1"><a href="./1.html">1</a></li>'+HL+htmlRight+'<li page="'+set.num+'"><a href="./'+set.num+'.html">'+set.num+'</a></li>';
						}else{
							html += '<li page="1"><a href="./1.html">1</a></li>'+HL+htmlC+HL+'<li page="'+set.num+'"><a href="./'+set.num+'.html">'+set.num+'</a></li>';
						}
						break;
					case "prev":
						if(i<=4){
						...
						break;
					case "«" :
						html+='<li class="active" page="1"><a href="./1.html">1</a></li>'+htmlLeft+HL+'<li page="'+set.num+'"><a href="./'+set.num+'.html">'+set.num+'</a></li>';
						break;
					case "»" :
						html+='<li page="1"><a href="./1.html">1</a></li>'+HL+htmlRight+'<li class="active" page="'+set.num+'"><a href="./'+set.num+'.html">'+set.num+'</a></li>';
						break;
					case "jump" :
						if(i<=4){
							if(i==1){
				...
				html += '<li><a>下一页</a></li>\
					<li><a  href="./'+set.num+'.html" aria-label="Next">&raquo;</a></li>';
		...
		if(set.num<=1){
			...
			n = parseInt(set.num);
			var html='<li><a  href="./1.html" aria-label="Previous">&laquo;</a></li>\
					<li><a>上一页</a></li>';
			for(var i=1;i<=n;i++){
				if(i==set.startnum){
					html+='<li class="active" page="'+i+'"><a href="./'+i+'.html">'+i+'</a></li>';
				}else{
					html+='<li page="'+i+'"><a href="./'+i+'.html">'+i+'</a></li>';
				}
			}
			// alert(html);
			// alert($("#page li.active a").html());
			html +='<li><a>下一页</a></li>\
					<li><a href="./'+set.num+'.html" aria-label="Next">&raquo;</a></li>';
			set.elem.html(html);
			clickpages.init();
