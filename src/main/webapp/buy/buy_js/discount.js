$(function () {
  ...
  $.ajax({
    type: 'get',
    url: CRMAPI + '/DiscountsCoursePrice/getDiscountsCoursePriceType', //正式环境
    dataType: "json",
    timeout:5000,
    success: function (data) {
      ...
      $(data.coursePriceVos).each(function (i, v) {
        b+=  '<div class="systemList">'+
          ...
          '<p class="F20 coupon">寒假联报 <img src="buy_images/personal/hui.gif" alt=""></p>'+
          '<p class="F16 keshi"><span class="color_red F22">' + (parseInt(v.length) - 40)+ '</span>课时+<span class="color_red">40</span>课时</p>'+
          '<p class="F20" style="display: none"><span class="period">' + v.length +'</span></p>'+
          '<p class="F14 UnLine" style="display: none;">原价：<span class="old">'+ v.originalPrice +'</span><span class="old0 none">' + v.nowPrice +'</span>元</p>'+
          '<p><span class="new F20 color_red">' + v.nowPrice +'</span>元</p>'+
          '<hr>'+
  ...
  $('body').on('click','#myTab li a',function () {
    ...
    $.ajax({
      type: 'get',
      url: CRMAPI + '/DiscountsCoursePrice/getDiscountsCoursePriceByType',
      dataType: "json",
      data:{type:type},
      timeout:5000,
      success: function (data) {
